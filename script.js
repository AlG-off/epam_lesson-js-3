...
function calcStr(str) {//Вычисляет выражение из строки
	str = str.replace(/[^-+*/.\d=]/g, '').replace(/(\d-)+?/g, "$1 ");/*Выкидываю из строки лишние символы и добавляю
	пробел после для правильного разбиения на операторы и операнды*/
	var arr = str.match(/(-?\d+(\.\d+)?|[-+/*])(?=.*=)/g);/*Выбираем отрицательные/положительные целые или дробные числа, 
	или оператор. Возращаем массив из всех совпадений*/
	if(!arr || !arr.length) return null; //Т.к в случае, если совпадений не было, match возвращает null, то проверяем на наличие совпадений
	var result = +arr[0];
	...
	for(var i=0; (2*i+1) < arr.length; i++) {
		...
		try{//Отлавливаем ошибку, если были введены некорректные данные
			...
			return "Ошибка ввода: Некорректная строка!";
		}
	}
